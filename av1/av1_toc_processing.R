...
toc_av1 = read.csv("toc/2023-12-26_sm_av1.csv") %>% janitor::clean_names()
mapping = read_xlsx("C:/Users/guil098/OneDrive - PNNL/Data Generation and Files/RC4/CN/01_RawData/20231031_Mapping_Raw_SCN_SBR_RC4_AV1_R1-48.xlsx")
...
toc_samples = 
  ...
  select(Randomized_ID, toc_percent, tn_percent, Sample_ID) %>% 
  mutate(Parent_ID = str_extract(Sample_ID, ".{7}(?=_)"))
...
reps_n_analytical = 
  ...
  mutate(mean = mean(c(rep, n_percent)),
         ...
         cv = round(sd/mean,3),
         range = max(n_percent, na.rm = T)-min(n_percent, na.rm = T))
...
reps_n_by_site = 
  ...
  summarise(mean = mean(tn_percent),
            ...
            cv = round(sd/mean,3),
            range = max(tn_percent, na.rm = T)-min(tn_percent, na.rm = T))
...
tn_cv <- ggplot(data = reps_n_by_site, aes(x = cv)) +
  geom_histogram(binwidth = 0.05, fill = "lightblue", color = "black", alpha = 0.7) +
  labs(x = "CV", y = "Frequency", title = "TN CV Distribution") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 16, face = "bold"), 
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 14, face = "bold"))
...
toc_reps_plot <- ggplot(data = toc_samples, aes(x = Parent_ID, y = toc_percent))+
  geom_point()

tn_reps_plot <- ggplot(data = toc_samples, aes(x = Parent_ID, y = tn_percent))+
  geom_point()

tn_mean_cv <- ggplot(data = reps_n_by_site, aes(x = mean, y = cv))+
  geom_point()
