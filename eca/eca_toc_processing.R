...
# Sample Mapping --------------------------------------------------------------
mapping <- read_xlsx("C:/Users/guil098/OneDrive - PNNL/Data Generation and Files/ECA/CN/01_RawData/20230628_Mapping_SCN_EC_EC_R1-288.xlsx")%>% 
  janitor::clean_names() %>% 
...
# RUN-3 ONLY --------------------------------------------------------------
## full run of batch 1 ECA samples 
...
#joining mapping with full run
toc_mapping <- mapping %>% 
  full_join(toc_samples3) %>% 
  filter(!is.na(toc_percent)) %>% 
